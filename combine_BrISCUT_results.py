...
#date='080918'
#ci = 0.95
genes = ['CDKN2A','TERT','MYC','BAP1','TERC','TP53','ARID1A','EGFR']
arms = [str(i)+'p' for i in range(1,21) if i not in [13,14,15]] + [str(i)+'q' for i in range(1,21) if i not in [13,14,15]] + ['13','14','15','21','22']
#arms = ['17p','3p','8p']
...
def make_table_results(folder, kspvals):
    ci = folder.split('_')[-1]
   ...
   # for tt in ['PANCAN']:
        ...
        x =  filter(r.search,os.listdir(folder+'/'+tt))
        if len(x)>0:
            for f in x: #all files
                ...
                if iter>1:
                    ...
                    #print all_previous
                    if any([kspvalsdf.loc[previous_index,'by'] > 0.05 for previous_index in all_previous]) \
                            or any([d.empty for d in [pd.read_csv(folder+'/'+tt+'/'+e) for e in all_previous_files]]):
                        print forindex
                        df['ksby'] = 1
                        kspvalsdf.set_value(forindex,'by',1)
                #END pruning
                df = df[df['ksby']<=0.05] #empty df if not significant.
            ...
            for rn,n in zip([rnk,tsrnk,oncrnk,aprnk,dprnk,anrnk,dnrnk],names):
                rnz = rn.sort('combined_sig',ascending=False)
...
def calc_overlaps(folder):
    genelocs = pd.read_csv('/cga/meyerson/home/jshih/aneuploidy/docs/hg19_genes_refseq_telcentfiltered_020518.txt',
                           sep='\t')
                ...
                for i in within.index:
                    ...
                    for j in newwithout.index:
                        ...
                        if overlap != False:
                            ...
                            peakgenelocs = peakgenelocs.sort('Start')
    ...
    dfdf = dfdf.sort('combined_sig_sum',ascending=False)
...
def overlap_significance(folder, overlapsdf, num_perms = 1000):
    info = pd.read_csv('/cga/meyerson/home/jshih/aneuploidy/docs/SNP6_hg19_chromosome_locs_200605.txt', sep='\t',
    ...
    for arm, type in fullcomboset:
        ...
        results = results.sort(['type','iter'])
        #print results
...
def make_column_results(folder):
...
def process_for_ggplot(results):

    locs = pd.read_csv('/cga/meyerson/home/jshih/aneuploidy/docs/SNP6_hg19_chromosome_locs_200605.txt',sep='\t',index_col='chromosome_info')
...
def process_for_ggplot_jagged(results):
    locs = pd.read_csv('/cga/meyerson/home/jshih/aneuploidy/docs/SNP6_hg19_chromosome_locs_200605.txt', sep='\t',
                       index_col='chromosome_info')
    #for tt in ['PANCAN']:
...
def make_all_peaks(results):
    ...
    for tt in os.listdir(results):
        try:
            ...
            briscut = briscut[briscut['ksby'] <= 0.05]
            ...
            newdf =newdf[['peak_location','cytoband','STOP_or_GO','direction','negpos','color','combined_sig','log10_ksby','ks_stat',
                          ...
                          'driver_score']]
            newdf = newdf.sort('combined_sig',ascending=False)
...
def filter_BrISCUT_knowngenes(folder, genes):
    ...
    for g in genes:
        try:
            minidf = df[df['Gene']==g]
            minidf = minidf.sort(columns=['combined_sig','n_events'],ascending=[False,False])
    ...
    for g in arms:
        ...
        for d in ['_multiter_BrISCUT_results','_multiter_BrISCUT_fig2','_firstiter_BrISCUT_fig2','_BrISCUT_fig2','_firstiter_BrISCUT_results','_BrISCUT_results']:
           ...
           # print onco
            ...
            for mini in [onco,ts]:
                if not mini.empty:
                    mini = mini.sort(columns=['combined_sig','n_events'],ascending=[False,False])
...
def extract_cols(folder, lop, arms):
 ...
 #       li = sorted(list_of_minicols, key=itemgetter(3),reverse=True)
        ...
        print df
        df = df.transpose().sort(3,ascending=False).transpose()
 #       df = df.transpose()
...
def all_processing(date, ci, genes=genes, arms=arms):
    folder='results_'+date+'_'+str(ci)
    kspval=folder+'/KS_pvalues_'+date+'_'+str(ci)+'.txt'
 
    make_table_results(folder,kspval)
    calc_overlaps(folder)
    lop = make_column_results(folder)
    process_for_ggplot(folder)
    process_for_ggplot_jagged(folder)
    #make_all_peaks(folder)
