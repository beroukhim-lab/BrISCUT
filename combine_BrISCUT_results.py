...
#date='080918'
#ci = 0.95
genes = ['CDKN2A','TERT','MYC','BAP1','TERC','TP53','ARID1A','EGFR', 'PPM1D']
...
def calc_overlaps(folder):
    genelocs = pd.read_csv('docs/hg19_genes_refseq_telcentfiltered_020518.txt',
                           sep='\t')
...
def process_for_ggplot(results,infoloc):

    locs = pd.read_csv(infoloc,sep='\t',index_col='chromosome_info')
...
def process_for_ggplot_jagged(results,infoloc):
    locs = pd.read_csv(infoloc, sep='\t',
