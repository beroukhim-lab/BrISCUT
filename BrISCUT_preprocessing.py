...
# this will need to incorporate edit_breakpoint_files_101719
...
def do_arm(arm, fileloc, tumortype, aneu, date, infoloc, threshold=0.2):
    try:
        info = pd.read_csv(infoloc,sep='\t',index_col='chromosome_info').transpose().to_dict()
        ...
        if not os.path.isdir('breakpoint_files_'+date+'/'+ tumortype): os.mkdir('breakpoint_files_'+date+'/'+tumortype)
        psedftel = get_percent_start_end(aneu, arm, coord, fileloc,threshold,'tel',False)
        psedfcent = get_percent_start_end(aneu, arm, coord, fileloc,threshold,'cent',False)
...
def get_percent_start_end(aneu, arm, coord, segdfloc,ncutoff,telcent,integer):
    ...
    for sample in allthesamples:
        ...
        alt_segments_in_arm= filter(lambda x:x[2]==direction,segments_in_arm) #alt segments
