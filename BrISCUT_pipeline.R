...
#segfiles need to correspond to ttlist. for example, c('KIRC.seg.txt','LUSC.seg.txt') and c('KIRC','LUSC')
BrISCUT_pipeline <- function(segfiles,ttlist,thedate,ci,infoloc,threshold) {
  #RESULTS FOLDER
  ...
  take_care_arms(segfile,tt,thedate,infoloc,threshold)
  all_arms <- 1:22
  all_arms <- c(unlist(lapply(all_arms,foop)),c('13','14','15','21','22'),unlist(lapply(all_arms,fooq)))
  parallelize_tt <- function(n) {
    segfile <- segfiles[n]
    t <- ttlist[n]
    take_care_arms(segfile,t,thedate,infoloc)
    for (a in all_arms) {
      for (d in c('amp','del')) {
  ...
  parSapply(clus, seq(length(ttlist)), parallelize_tt)
  stopCluster(clus)
